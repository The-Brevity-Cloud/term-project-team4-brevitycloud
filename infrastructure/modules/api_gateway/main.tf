...
# Find or create the Cognito JWT authorizer resource.
# Assuming cognito user pool is passed in via variables (var.cognito_user_pool_id, var.cognito_client_id)
# Check if an authorizer already exists for other routes, reuse if possible.
...
resource "aws_apigatewayv2_authorizer" "jwt_authorizer" {
  ...
  jwt_configuration {
    audience = [var.cognito_client_id] # Use the client ID passed as variable
    issuer   = "https://${var.cognito_issuer_url}" # Construct or pass issuer URL as variable
  }
}

# --- Rekognition Integration & Route ---
resource "aws_apigatewayv2_integration" "rekognition_integration" {
  api_id             = aws_apigatewayv2_api.api.id
  integration_type   = "AWS_PROXY"
  integration_uri    = var.invoke_rekognition_lambda_invoke_arn # Passed from rekognition module
  integration_method = "POST"
  payload_format_version = "2.0"
}
...
resource "aws_apigatewayv2_route" "rekognition_route" {
  ...
  target    = "integrations/${aws_apigatewayv2_integration.rekognition_integration.id}"
  # Add authorization
  authorization_type = "JWT"
  authorizer_id      = aws_apigatewayv2_authorizer.jwt_authorizer.id 
}

# Permission for API Gateway to invoke Rekognition Invoker Lambda
resource "aws_lambda_permission" "apigw_rekognition_invoke" {
  statement_id  = "AllowAPIGatewayInvokeRekognition"
  action        = "lambda:InvokeFunction"
  function_name = var.invoke_rekognition_lambda_name # Passed from rekognition module
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.api.execution_arn}/*/${aws_apigatewayv2_route.rekognition_route.route_key}"
}

# --- Transcribe Integration & Route ---
resource "aws_apigatewayv2_integration" "transcribe_integration" {
  api_id             = aws_apigatewayv2_api.api.id
  integration_type   = "AWS_PROXY"
  integration_uri    = var.invoke_transcribe_lambda_invoke_arn # Passed from transcribe module
  integration_method = "POST"
  payload_format_version = "2.0"
}
...
resource "aws_apigatewayv2_route" "transcribe_route" {
  ...
  target    = "integrations/${aws_apigatewayv2_integration.transcribe_integration.id}"
   # Add authorization
  authorization_type = "JWT"
  authorizer_id      = aws_apigatewayv2_authorizer.jwt_authorizer.id 
}

# Permission for API Gateway to invoke Transcribe Invoker Lambda
resource "aws_lambda_permission" "apigw_transcribe_invoke" {
  statement_id  = "AllowAPIGatewayInvokeTranscribe"
  action        = "lambda:InvokeFunction"
  function_name = var.invoke_transcribe_lambda_name # Passed from transcribe module
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.api.execution_arn}/*/${aws_apigatewayv2_route.transcribe_route.route_key}"
}
